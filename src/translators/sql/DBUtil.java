...
{	
	 ...
	 */
	public Connection connectToDB(String dbURI, String userName, String password) throws SQLException, ClassNotFoundException{
		...
		if(thisConnection != null){
			...
			println("(" + thisDBMetadata.getUserName() + ") are connected to (" + thisDBMetadata.getURL() + ") " + 
					thisDBMetadata.getDatabaseProductName() + " using " + thisDBMetadata.getDriverVersion());
	...
	public Connection connectToDB() throws SQLException, ClassNotFoundException{
		return this.connectToDB(POSTGRESQL_DEFAULT_URI, POSTGRESQL_DEFAULT_USER_NAME, POSTGRESQL_DEFAULT_PWD);
	...
	public void showResultSetDBData(ResultSet thisResultSet) throws SQLException{
		...
		thisColumnNameStringBuilder.append("\n").append(thisColumnTypeStringBuilder.toString().trim()).append("\n"); 
