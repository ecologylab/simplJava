...
public class DBSerializer
{
	...
	protected/* org.postgresql. */DBUtil	conn;
	...
	/**
	 * This creates an instance that can be used to serialize or deserialize a Java object from a
	 * PostgreSQL table.
	 */
	public DBSerializer(/* org.postgresql. */Connection c, String type) throws SQLException
	{
		try
		{
			// conn = c;
		...
		ResultSet rs = conn
				.ExecSQL("select typname from pg_type,pg_class where typname=relname and typname='" + type
						+ "'");
		if (rs != null)
		{
			if (rs.next())
		...
		/*
		 * if(!status) throw new PSQLException("postgresql.serial.table",type);
		 */
	 ...
	 * <p>
	 * 
	 * If the object has an int called OID, and it is > 0, then that value is used for the OID, and
	 * the table will be updated. If the value of OID is 0, then a new row will be created, and the
	 * value of OID will be set in the object. This enables an object's value in the database to be
	 * updateable.
	 * 
	 * If the object has no int called OID, then the object is stored. However if the object is later
	 * retrieved, amended and stored again, it's new state will be appended to the table, and will not
	 * overwrite the old entries.
	 * 
	 * @param o
	 *          Object to store (must implement Serializable)
	...
	public int store(Object o) throws SQLException
	{
		try
		{
			...
			StringBuffer sb = new StringBuffer(update ? "update " + tableName + " set" : "insert into "
					+ tableName + " values ");
			char sep = update ? ' ' : '(';
			for (int i = 0; i < f.length; i++)
			{
	...
	/**
	 * This method is not used by the driver, but it creates a table, given a Serializable Java
	 * Object. It should be used before serializing any objects.
	 * 
	 * @param c
	 *          Connection to database
	 * @param o
	 *          Object to base table on
	 * @exception SQLException
	 *              on error
	 */
	 ...
	 *          Connection to database
	 * @param o
	 *          Class to base table on
	 * @throws Exception
	 */
	public static void create(/* org.postgresql. *//* Connection */DBUtil con, Class c)
			throws Exception
	{
		if (c.isInterface())
	...
	/**
	 * This converts a Java Class name to a org.postgresql table, by replacing . with _
	 * <p>
	 * 
	 * Because of this, a Class name may not have _ in the name.
	 * <p>
	 * Another limitation, is that the entire class name (including packages) cannot be longer than 32
	 ...
	 * @throws Exception
	 */
	public static String toPostgreSQL(String name) throws Exception
	{
		name = name.toLowerCase();

		if (name.indexOf("_") > -1)
			// throw new PSQLException("postgresql.serial.underscore");
			throw new Exception("postgresql.serial.underscore");

		if (name.length() > 32)
			// throw new PSQLException("postgresql.serial.namelength",name,new Integer(name.length()));
			throw new Exception("postgresql.serial.namelength");
