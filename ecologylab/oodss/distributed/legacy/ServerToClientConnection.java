...
public class ServerToClientConnection extends Debug implements Runnable, ServerConstants
{
    ...
    {
        char[] ch_array = new char[DEFAULT_MAX_MESSAGE_LENGTH_CHARS];
        int count = 0;

        while (count < DEFAULT_MAX_MESSAGE_LENGTH_CHARS)
        {
            int c = in.read();
            if (c == -1)
     ...
     * 
     * See ecologylab.oodss.Shutdownable#shutdownAndNotify(java.util.Collection)
     */
    public void shutdown(Collection<Object> objectsToBeNotified)
    {
		...
		/**
		 * @see ecologylab.oodss.distributed.common.ServerConstants#getLastActivity()
		 */
