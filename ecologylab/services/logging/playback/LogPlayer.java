...
public abstract class LogPlayer<OP extends MixedInitiativeOp, LOG extends Logging<OP>>
        extends ApplicationEnvironment implements ActionListener,
        WindowListener, PlaybackControlCommands, Runnable
{
    ...
    /**
     * Modes: 0 = no file selected (need to get a file name). 1 = file selected
     * and loading. 2 = file loaded, display log.
     */
    ...
    /**
     * The number of milliseconds between ops when the log is playing. Setting
     * to -1 will try to use op timestamps.
     */
    ...
    /**
     * Translates the given file to XML. Necessary because it is not possible to
     * cast directly to generic types.
     * 
     * @param logToRead
     * @return
        ...
        if (e.getActionCommand() == null)
        {
            if (playing)
            {
                ...
                { // if we're basing off the interval, we have to check some
                    // stuff
                    long delay = log.getNext().getSessionTime()
                            - log.getCurrentOp().getSessionTime();
    ...
    /**
     * Used to be thread safe with Swing. Either sets up the UI, or switches it,
     * depending on whether or not a file has been selected.
     * 
    ...
    /**
     * Sends the server notification that we are logging-out, then shuts down
     * the program.
     */
