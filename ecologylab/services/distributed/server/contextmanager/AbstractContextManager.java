...
/**
 * The base class for all ContextManagers, objects which track the state and respond to clients on a server. There is a
 * one-to-one correspondence between connected clients and ContextManager instances.
 * 
 * AbstractContextManager handles all encoding and decoding of messages, as well as translating them. Hook methods
 * provide places where subclasses may modify behavior for specific purposes.
 * 
 * Typical usage is to have the context manager's request queue be filled by a network thread, while it is emptied by a
 * working thread.
 * 
 * The normal cycle for filling the queue is to call acquireIncomingSequenceBuf() to clear and get the
 * incomingCharBuffer, then fill it externally (normally passing it as an argument to a CharsetDecoder.decode call),
 * then calling processIncomingSequenceBufToQueue() to release it and let the ContextManager store the characters,
 * converting messages into objects as they become available.
 * 
...
public abstract class AbstractContextManager extends Debug implements ServerConstants
{
	...
	/**
	 * Extracts messages from the given CharBuffer, using HTTP-like headers, converting them into RequestMessage
	 * instances, then enqueues those instances.
	 * 
	 * enqueueStringMessage will normally be called repeatedly, as new data comes in from a client. It will automatically
	 * parse messages that are split up over multiple reads, and will handle multiple messages in one read, if necessary.
	 * 
	 * @param message
	 *           the CharBuffer containing one or more messages, or pieces of messages.
	 */
	public final void processIncomingSequenceBufToQueue(CharBuffer incomingSequenceBuf) throws CharacterCodingException, BadClientException
	{
		synchronized (msgBufIncoming)
		{
			...
			while (msgBufIncoming.length() > 0)
			{
				...
				if (endOfFirstHeader == -1)
				{ /*
					 * no header yet; if it's too large, bad client; if it's not too large yet, just exit, it'll get checked
					 * again when more data comes down the pipe
					 */
					if (msgBufIncoming.length() > ServerConstants.MAX_HTTP_HEADER_LENGTH)
					{
						// clear the buffer
						BadClientException e = new BadClientException(this.socket.socket().getInetAddress().getHostAddress(),
								"Maximum HTTP header length exceeded. Read " + msgBufIncoming.length() + "/"
										+ MAX_HTTP_HEADER_LENGTH);
				...
				{ // we've read all of the header, and have it loaded into the map;
					...
					if (contentLengthRemaining == -1)
					{
						try
						{
							...
							contentUid = Long.parseLong(this.headerMap.get(UNIQUE_IDENTIFIER_STRING));
	...
	/**
	 * Appends the sender's IP address to the incoming message and calls performService on the given RequestMessage using
	 * the local ObjectRegistry.
	 * 
	 ...
	 *           TODO
	 */
	private final void processRequest(MessageWithMetadata<RequestMessage> requestWithMetadata)
	{
		...
		else
		{
			...
			{ // if the response is null, then we do
				...
				try
				{
					...
					this.createHeader(msgBufOutgoing, headerBufOutgoing, request, response, requestWithMetadata.getUid());
					...
					msgBufOutgoing.getChars(0, msgBufOutgoing.length(), outgoingChars.array(), headerBufOutgoing.length());
