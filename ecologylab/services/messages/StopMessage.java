 ...
 * @author blake
 */
@simpl_inherit public class StopMessage extends RequestMessage implements
		SessionObjects
{
	/**
	 * This message doesn't DO anything. It just indicates that a stop (or
	 * shutdown) event should occur.
	 */
	@Override public ResponseMessage performService(Scope objectRegistry)
	{
		StartAndStoppable sas = (StartAndStoppable) objectRegistry
				.get(MAIN_START_AND_STOPPABLE);
	 ...
	 * without extending that. Stop the server, once we confirm processing of our
	 * Stop message.
	 * 
	...
	{
		ServicesClient browserServicesClient = (ServicesClient) objectRegistry
				.get(BROWSER_SERVICES_CLIENT);
