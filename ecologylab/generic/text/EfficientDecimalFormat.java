 ...
 * {@link java.text.DecimalFormat} class, as described below, except that it:
 * 
 ...
 * default locale, call one of <code>NumberFormat</code>'s factory methods,
 * such as <code>getInstance()</code>. In general, do not call the
 ...
 * symbols are stored in a <code>DecimalFormatSymbols</code> object. When
 * using the <code>NumberFormat</code> factory methods, the pattern and
 * symbols are read from localized <code>ResourceBundle</code>s.
 * 
 * <h4>Patterns</h4>
 * 
 ...
 * optional; if absent, then the positive subpattern prefixed with the localized
 * minus sign (<code>'-'</code> in most locales) is used as the negative
 * subpattern. That is, <code>"0.00"</code> alone is equivalent to
 * <code>"0.00;-0.00"</code>. If there is an explicit negative subpattern, it
 * serves only to specify the negative prefix and suffix; the number of digits,
 * minimal digits, and other characteristics are all the same as the positive
 * pattern. That means that <code>"#,##0.0#;(#)"</code> produces precisely the
 * same behavior as <code>"#,##0.0#;(#,##0.0#)"</code>.
 * 
 * <p>
 ...
 * single quote itself, use two in a row: <code>"# o''clock"</code>. </table>
 * </blockquote>
 * 
 ...
 * suffix from the pattern. This allows patterns such as
 * <code>"0.###E0 m/s"</code>.
 * 
 ...
 * <em>engineering notation</em>, in which the exponent is a multiple of
 * three, e.g., <code>"##0.#####E0"</code>. Using this pattern, the number
 * 12345 formats to <code>"12.345E3"</code>, and 123456 formats to
 * <code>"123.456E3"</code>.
 * 
 * <li>Otherwise, the minimum number of integer digits is achieved by adjusting
 * the exponent. Example: 0.00123 formatted with <code>"00.###E0"</code>
 ...
 * <code>"##0.##E0"</code> is <code>"12.3E3"</code>. To show all digits,
 * set the significant digits count to zero. The number of significant digits
 ...
 * <code>&#92;u221E</code>, with the positive or negative prefixes and
 * suffixes applied. The infinity character is determined by the
...
public class EfficientDecimalFormat extends DecimalFormat
{
	 ...
	 * minor adjustments to a standard format, you can modify the format returned
	 ...
	 */
	public EfficientDecimalFormat(String pattern, DecimalFormatSymbols symbols)
	{
		// Always applyPattern after the symbols are set
	 ...
	 */
	public final Appendable format(Object number, Appendable toAppendTo)
			throws IOException
	{
		...
		{
			return format((BigDecimal) number, toAppendTo);
		}
		else if (number instanceof BigInteger)
		{
			return format((BigInteger) number, toAppendTo);
		}
	 ...
	 * @throws IOException
	 */
	public Appendable format(double number, Appendable result)
			throws IOException
	{
		 ...
		 * exponent), but a negative sign bit. It is semantically distinct from a
		 * zero with a positive sign bit, and this distinction is important to
		 * certain kinds of computations. However, it's a little tricky to detect,
		 * since (-0.0 == 0.0) and !(-0.0 < 0.0). How then, you may ask, does it
		 * behave distinctly from +0.0? Well, 1/(-0.0) == -Infinity. Proper
		 * detection of -0.0 is needed to deal with the issues raised by bugs
		 * 4106658, 4106667, and 4147706. Liu 7/6/98.
		 */
		...
		synchronized (digitList)
		{
			...
			digitList.set(number, useExponentialNotation ? maxIntDigits
					+ maxFraDigits : maxFraDigits, !useExponentialNotation);
			return subformat(result, isNegative, false, maxIntDigits,
	 ...
	 * @see java.text.FieldPosition
	 */
	public Appendable format(long number, Appendable result) throws IOException
	{
		...
		if (useBigInteger)
		{
			...
			return format(bigIntegerValue, result, true);
		}
		...
		synchronized (digitList)
		{
			...
			return subformat(result, isNegative, true, maxIntDigits, minIntDigits,
	 ...
	 * @throws IOException
	 */
	public Appendable format(BigDecimal number, Appendable result)
			throws IOException
	{
		if (multiplier != 1)
		{
		...
		synchronized (digitList)
		{
			...
			digitList
					.set(
							number,
							useExponentialNotation ? ((maximumDigits < 0) ? Integer.MAX_VALUE
									: maximumDigits)
	 ...
	 * @see java.text.FieldPosition
	 */
	private Appendable format(BigInteger number, Appendable result)
			throws IOException
	{
		return format(number, result, false);
	}
	 ...
	 */
	private Appendable format(BigInteger number, Appendable result,
			boolean formatLong) throws IOException
	{
		if (multiplier != 1)
		{
	 ...
	 * @throws IOException
	 */
	private Appendable subformat(Appendable result, boolean isNegative,
			boolean isInteger, int maxIntDigits, int minIntDigits,
			int maxFraDigits, int minFraDigits) throws IOException
	{
		...
		else
		{
			...
			// indicate whether or not any characters have been appended before the
			// fraction part
			boolean actuallyAppended = (count - 1 < 0);
			...
			if (!fractionPresent && actuallyAppended)
			{
				result.append(zero);
			}
	 ...
	 * string), and the parsed number is returned. The updated <code>pos</code>
	 ...
	 * they are written: <code>"17"</code> and <code>"17.000"</code> both
	 * parse to <code>Long(17)</code>. Values that cannot fit into a
	 * <code>Long</code> are returned as <code>Double</code>s. This includes
	 * values with a fractional part, infinite values, <code>NaN</code>, and
	 * the value -0.0. <code>DecimalFormat</code> does <em>not</em> decide
	 * whether to return a <code>Double</code> or a <code>Long</code> based
	 * on the presence of a decimal separator in the source string. Doing so
	 * would prevent integers that overflow the mantissa of a double, such as
	...
	{
		// special case NaN
		if (text.regionMatches(pos.getIndex(), symbols.getNaN(), 0, symbols
				.getNaN().length()))
		{
			...
			if (multiplier != 1)
			{
				...
				{ // non-terminating decimal expansion
					bigDecimalResult = bigDecimalResult.divide(
							getBigDecimalMultiplier(), BigDecimal.ROUND_HALF_EVEN);
				}
			}
	 ...
	 *           integer only.
	 * @param status
	 *           Upon return contains boolean status flags indicating whether the
			...
			boolean isExponent, boolean status[])
	{
		...
		if (!isExponent
				&& text.regionMatches(position, symbols.getInfinity(), 0, symbols
						.getInfinity().length()))
		{
		...
		else
		{
			...
			for (; position < text.length(); ++position)
			{
				 ...
				 * '0'..'9'. We do so by using the Character.digit() method, which
				 * converts a valid Unicode digit to the range 0..9.
				 * 
				 * The character 'ch' may be a digit. If so, place its value from 0
				 * to 9 in 'digit'. First try using the locale digit, which may or
				 * MAY NOT be a standard Unicode digit range. If this fails, try
				 * using the standard Unicode digit ranges by calling
				 * Character.digit(). If this also fails, digit will have a value
	 ...
	 */
	@Override public String getPositivePrefix()
	{
	 ...
	 * Examples: +123, $123, sFr123
	 */
	@Override public void setPositivePrefix(String newValue)
	{
	 ...
	 */
	@Override public String getNegativePrefix()
	{
	 ...
	 * Examples: -123, ($123) (with negative suffix), sFr-123
	 */
	@Override public void setNegativePrefix(String newValue)
	{
	 ...
	 * Example: 123%
	 */
	@Override public String getPositiveSuffix()
	{
	 ...
	 * Example: 123%
	 */
	@Override public void setPositiveSuffix(String newValue)
	{
	 ...
	 */
	@Override public String getNegativeSuffix()
	{
	 ...
	 * Examples: 123%
	 */
	@Override public void setNegativeSuffix(String newValue)
	{
	 ...
	 * '%' (for Arabic, use the Arabic percent sign). For a per mille format, set
	 * the multiplier to 1000 and the suffixes to have '&#92;u2030'.
	 * 
	 * <P>
	 ...
	 * @see #getMultiplier
	 */
	@Override public void setMultiplier(int newValue)
	{
		multiplier = newValue;
	 ...
	 */
	@Override public int getGroupingSize()
	{
		return groupingSize;
	}
	 ...
	 */
	@Override public void setGroupingSize(int newValue)
	{
	 ...
	 */
	@Override public boolean isDecimalSeparatorAlwaysShown()
	{
	 ...
	 * Example: Decimal ON: 12345 -> 12345.; OFF: 12345 -> 12345
	 */
	@Override public void setDecimalSeparatorAlwaysShown(boolean newValue)
	{
	 ...
	 * @since 1.5
	 */
	@Override public boolean isParseBigDecimal()
	{
	 ...
	 * @since 1.5
	 */
	@Override public void setParseBigDecimal(boolean newValue)
	{
	...
	@Override public Object clone()
	{
		try
		{
			EfficientDecimalFormat other = (EfficientDecimalFormat) super.clone();
	...
	@Override public boolean equals(Object obj)
	{
		...
		EfficientDecimalFormat other = (EfficientDecimalFormat) obj;
	 ...
	 * followed by another character; QUOTE may not occur by itself at the end of
	 * the pattern.
	 * 
	 * @param pattern
	 ...
	 * string will generate the same affix pattern (or literal affix) when passed
	 * to toPattern().
	 * 
	 * @param buffer
	 ...
	 * @param expAffix
	 *           a corresponding expanded affix, such as positivePrefix. Ignored
	 *           unless affixPattern is null. If affixPattern is null, then
	 *           expAffix is appended as a literal affix.
	 * @param localized
	 *           true if the appended pattern should contain localized pattern
	 *           characters; otherwise, non-localized pattern chars are appended
	 */
		...
		for (int j = 1; j >= 0; --j)
		{
			...
			for (i = digitCount; i > 0; --i)
			{
				...
				result.append(i <= getMinimumIntegerDigits() ? (localized ? symbols
						.getZeroDigit() : PATTERN_ZERO_DIGIT) : (localized ? symbols
						.getDigit() : PATTERN_DIGIT));
			}
			...
			if (j == 1)
			{
								...
								.equals(posSuffixPattern)))
				{
					if ((negPrefixPattern != null && posPrefixPattern != null && negPrefixPattern
							...
							|| (negPrefixPattern == posPrefixPattern && // n == p ==
							// null
	...
	@Override public void applyPattern(String pattern)
	{
		...
		for (int j = 1; j >= 0 && start < pattern.length(); --j)
		{
			 ...
			 * which allows proper round-trip patterns. That is, we don't want "#"
			 * to become "#0" when toPattern() is called (even though that's what
	 ...
	 * <code>BigDecimal</code> objects, the lower of <code>newValue</code>
	...
	public void setMaximumIntegerDigits(int newValue)
	{
		...
		super
				.setMaximumIntegerDigits((maximumIntegerDigits > DOUBLE_INTEGER_DIGITS) ? DOUBLE_INTEGER_DIGITS
						: maximumIntegerDigits);
		if (minimumIntegerDigits > maximumIntegerDigits)
		{
			minimumIntegerDigits = maximumIntegerDigits;
			super
					.setMinimumIntegerDigits((minimumIntegerDigits > DOUBLE_INTEGER_DIGITS) ? DOUBLE_INTEGER_DIGITS
	...
	@Override public void setMaximumFractionDigits(int newValue)
	{
		...
		super
				.setMaximumFractionDigits((maximumFractionDigits > DOUBLE_FRACTION_DIGITS) ? DOUBLE_FRACTION_DIGITS
						: maximumFractionDigits);
		if (minimumFractionDigits > maximumFractionDigits)
		{
			minimumFractionDigits = maximumFractionDigits;
			super
					.setMinimumFractionDigits((minimumFractionDigits > DOUBLE_FRACTION_DIGITS) ? DOUBLE_FRACTION_DIGITS
	 ...
	 * number. For formatting numbers other than <code>BigInteger</code> and
	 * <code>BigDecimal</code> objects, the lower of the return value and 309
	 * is used.
	 * 
	 * @see #setMaximumIntegerDigits
	 */
	@Override public int getMaximumIntegerDigits()
	{
	 ...
	 */
	@Override public int getMinimumIntegerDigits()
	{
	 ...
	 * number. For formatting numbers other than <code>BigInteger</code> and
	 * <code>BigDecimal</code> objects, the lower of the return value and 340
	 * is used.
	 * 
	 * @see #setMaximumFractionDigits
	 */
	@Override public int getMaximumFractionDigits()
	{
	 ...
	 */
	@Override public int getMinimumFractionDigits()
	{
	 ...
	 * validations and adjustments for older serialized versions. The validations
	 * and adjustments are:
	 * <ol>
	 ...
	 * <li> If <code>serialVersionOnStream</code> is less than 1, indicating
	 * that the stream was written by JDK 1.1, initialize
	 * <code>useExponentialNotation</code> to false, since it was not present
	 * in JDK 1.1.
	 ...
	 * a number. Must be greater than 0 if <code>NumberFormat.groupingUsed</code>
	 * is true.
	 * 
	 * @serial
	 * @see #getGroupingSize
	 * @see java.text.NumberFormat#isGroupingUsed
	 */
	private byte								groupingSize						= 3;											// invariant,
	 ...
	 * @see java.text.DecimalFormatSymbols
	 */
	private DecimalFormatSymbols			symbols								= null;										// LIU
	 ...
	 * @since 1.2
	 */
	private boolean							useExponentialNotation;														// Newly
	 ...
	 * @since 1.2
	 */
	private byte								minExponentDigits;																// Newly
	 ...
	 * @since 1.5
	 */
	private int									maximumIntegerDigits				= super
	 ...
	 * @since 1.5
	 */
	private int									minimumIntegerDigits				= super
	 ...
	 * @since 1.5
	 */
	private int									maximumFractionDigits			= super
	 ...
	 * @since 1.5
	 */
	private int									minimumFractionDigits			= super
