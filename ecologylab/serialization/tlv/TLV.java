...
public class TLV
{
	...
	/*
	 * This class is used to parse DER encoded data (BER indefinite length is also supported) and
	 * assemble new DER data structures from elements. It is not safe to modify elements of
	 * constructed DER value that was created using parsing constructor or copy() method.
	 */
	...
	public TLV(byte[] buffer, int offset) throws TLVException
	{
		...
		try
		{
			...
			while (true)
			{
				if (indefinite && data[offset] == 0 && data[offset + 1] == 0)
				{
				...
				offset = (data[offset + 1] == (byte) 0x80 ? 2 : 0) + temp.valueOffset + temp.length;
	 ...
	 *          byte[] value
	 * @param offset
	 *          int offset in the p_data
	 * @param length
	 *          int number of bytes from the p_data
	 * @return new object
	 */
	 ...
	 *          byte[] value
	 * @param notUsed
	 *          int number of bytes from the p_data that are not used
	 ...
	 * @throws TLVException
	 *           if this TLV doesn't represent integer value
	 */
	 ...
	 * @return integer value
	 * @throws TLVException
	 *           if this TLV is not octet string
	 */
	...
	{
		return (type == TELETEXSTR_TYPE || type == PRINTSTR_TYPE || type == UNIVSTR_TYPE
				|| type == UTF8STR_TYPE || type == BMPSTR_TYPE);
	}
	...
	public Calendar getTime() throws TLVException
	{
		...
		if (type != GEN_TIME_TYPE && type != UCT_TIME_TYPE)
		{
	 ...
	 * 
	 * @param type
	 *          tag of optional value
	 * @return this object if type doesn't match or the next one if it does
	 */
	...
	public boolean checkFlag(int index) throws TLVException
	{
		...
		return ((data[valueOffset + 1 + (index / 8)] << index % 8) & 0x80) != 0;
	}
	...
	{
		return Utils.byteMatch(this.data, valueOffset, length, data, 0, data.length);
	}
	...
	private void print(PrintStream out, int level)
	{
		...
		if (child == null)
		{
			out.print("Type: 0x" + Integer.toHexString(type) + " length: " + length + " value: ");
			if (type == PRINTSTR_TYPE || type == TELETEXSTR_TYPE || type == UTF8STR_TYPE
					|| type == IA5STR_TYPE || type == UNIVSTR_TYPE)
			{
				try
				{
					out.print(new String(buffer, valueOffset, length, Utils.utf8));
				}
				catch (UnsupportedEncodingException e)
				{
					// ignore
		...
		else
		{
			...
			out.println("  (0x" + Integer.toHexString(type) + " " + length + ")");
	 ...
	 * @param t
	 *          TLV object
	 * @return true if both objects have the same type and contain the same data
	 */
	public boolean match(TLV t)
	{
		...
		return Utils.byteMatch(data, valueOffset, length, t.data, t.valueOffset, t.length);
	}
